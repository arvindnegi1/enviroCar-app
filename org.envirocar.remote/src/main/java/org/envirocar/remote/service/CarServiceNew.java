...
public interface CarServiceNew {
    ...
    Call<CarNew> getAllCarNew(@Path("manfuid")String manufid,@Path("carid")String carid);

    @GET("manufacturers/{manfuid}/vehicles/{carid}")
    Call<CarNew> getAllCarNewObservable(@Path("manfuid")String manufid,@Path("carid")String carid);
}
